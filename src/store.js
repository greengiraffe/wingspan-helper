...
const loadState = () => {
  ...
  if (localStorageAvailable()) {
    ...
    if (
      !state ||
      typeof state !== "object" ||
      defaultState.version !== state.version
    ) {
      localStorage.removeItem(stateStorageKey);
      state = defaultState;
    }
...
const defaultState = {
  ...
  playerCount: 5,
  scoreTypes,
  localizedScoreTypes: i18n.global.tm("scoreTypes"),
...
export const store = createStore({
  ...
  getters: {
    activePlayers: (state) => {
      return Object.fromEntries(
        Object.entries(state.players).slice(0, state.playerCount)
      );
    },
  ...
  mutations: {
    setLanguage(state, payload) {
      ...
      state.localizedScoreTypes = i18n.global.tm("scoreTypes");
    },
